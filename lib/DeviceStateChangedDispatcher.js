...
class DeviceStateChangeDispatcher
{
    ...
    constructor(app)
    {
        ...
        this.subscribeQueue = Promise.resolve();
        this._init();
    }
    ...
    // Serialise subscriptions: bursts of parallel makeCapabilityInstance calls make the Homey API
    // manager exceed its 10s subscribe timeout.
    _enqueueSubscribe(task)
    {
        const result = this.subscribeQueue.then(task, task);
        this.subscribeQueue = result.then(() => {}, () => {});
        return result;
    }
    ...
    async registerDeviceCapability(device, capabilityId)
    {
        ...
        try
        {
            ...
            const listener = await this._enqueueSubscribe(() => device.makeCapabilityInstance(capabilityId, (value) => this._handleStateChange(device, value, capabilityId)));
    ...
    async removeDevice(deviceOrId)
    {
        const deviceId = typeof deviceOrId === 'string' ? deviceOrId : deviceOrId && deviceOrId.id;
        if (!deviceId)
        {
            return;
        }
        ...
        for (const [key, listener] of this.listeners.entries())
        {
            ...
            {
                this.app.updateLog(`removeDevice: ${error.message}`, 0);
            }

            this.listeners.delete(key);
            this.expectedListenerKeys.delete(key);
            this.lastValues.delete(key.replace('::', '_'));
        }
    }
