...
class DeviceStateChangeDispatcher
{
    ...
    async registerDeviceCapability(device, capabilityId)
    {
        ...
        {
            this.app.updateLog('Error registering device capability: missing device or capabilityId');
            return;
        }
        ...
        if (!device)
        {
            this.app.updateLog('Error registering device capability: device not found');
            return;
        }
        ...
        for (const driver of Object.values(drivers))
        {
            ...
            for (let device of Object.values(devices))
            {
                if (device.checkStateChange)
                {
                    ...
                    {
                        this.app.updateLog(`_handleStateChange: ${error.message}`);
                    }
                }
