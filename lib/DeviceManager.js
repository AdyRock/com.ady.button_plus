...
class DeviceManager
{
    ...
    getDeviceId(device)
    {
        ...
        this.app.updateLog('Device id not found');
    ...
    async getDeviceById(deviceId)
    {
        if (deviceId)
        {
            ...
            if (!device)
            {
                this.app.updateLog(`Device not found: ${deviceId}`);
            }
            return device;
        }
    ...
    async registerDevice(device)
    {
        if (typeof device === 'object')
        {
            if (!device.id)
            {
                this.app.updateLog('[SKIP] No device id provided');
                return;
            }
            ...
            if (!deviceName)
            {
                this.app.updateLog('[SKIP] No device name provided');
                return;
            }

            if (this.isRegistered(device))
            {
                this.app.updateLog('device already registered');
                return;
            }
